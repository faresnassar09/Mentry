...
<nav class="bg-white shadow-sm py-4 h-auto">
    <div class="max-w-7xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between"> 
        <div class="text-2xl font-bold text-blue-600 mb-4 sm:mb-0"> 
        ...
        <div class="flex flex-wrap justify-center sm:justify-end items-center gap-4 sm:gap-6 w-full sm:w-auto"> 
            <div x-data="{ open: false }" class="relative w-full sm:w-auto order-last sm:order-first">
                <button @click="open = !open" type="button" class="inline-flex items-center gap-x-1 text-sm font-medium text-gray-700 hover:text-blue-600 transition duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 rounded-md py-2 px-3 w-full justify-center sm:justify-start">
            ...
            @if (!auth()->check())
                <a href="{{ route('login') }}"
                   class="text-gray-700 hover:text-blue-600 font-medium transition duration-200 order-first sm:order-none"> {{-- تغيير ترتيب الأزرار --}}
                    {{ __('messages.login_button') }}
                </a>
                <a href="{{ route('register') }}"
                   class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 transition duration-200 order-first sm:order-none"> {{-- تغيير ترتيب الأزرار --}}
                    {{ __('messages.create_account_button') }}
                </a>
            @else
                <a href="{{ route('dashboard') }}"
                   class="text-gray-700 hover:text-blue-600 font-medium transition duration-200 order-first sm:order-none">
