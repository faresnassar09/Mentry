...
@section('title', __('messages.edit_note_page_title'))
@section('content')
...
<form action="{{route('users.study.notes.update',$note->id)}}" method="POST" class="space-y-6 max-w-xl mx-auto bg-white p-6 rounded-xl shadow">
    ...
    <div>
        ...
        <textarea
            ...
            rows="6"
            required
            class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring focus:ring-blue-300 @error('body') border-red-500 @enderror">{{ old('body', $note->body) }}</textarea>
        @error('body')
        <div class="text-red-600 text-sm">{{ $message }}</div>
        @enderror
    </div>
    ...
    <div class="text-center">
        <button type="submit"
            ...
            {{ __('messages.update_note_button') }}
        </button>
    </div>
</form>
