...
@section('title', __('messages.add_material_page_title'))
@section('content')
...
<form action="{{route('users.study.materials.insert')}}" method="POST" enctype="multipart/form-data" class="space-y-6 p-6 bg-white rounded-xl shadow-md max-w-lg mx-auto">
    ...
    <h2 class="text-sm font-semibold text-gray-800 text-center">{{ __('messages.material_section_description') }}</h2>

    <div>
        <label for="title" class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.material_title_label') }}</label>
        <input
            type="text"
            name="title"
            id="title"
    ...
    <div>
        <label class="block text-sm font-medium text-gray-700 mb-2">{{ __('messages.material_type_label') }}</label>
        <div class="flex items-center gap-6">
            <label class="inline-flex items-center">
                <input type="radio" name="type" value="1" checked required class="text-blue-600 focus:ring-blue-500">
                <span class="ml-2 text-gray-700">{{ __('messages.material_type_lecture') }}</span>
            </label>
            <label class="inline-flex items-center">
                <input type="radio" name="type" value="2" required class="text-blue-600 focus:ring-blue-500">
                <span class="ml-2 text-gray-700">{{ __('messages.material_type_summary') }}</span>
            </label>
        </div>
    </div>

    <div>
        <label for="file" class="block text-sm font-medium text-gray-700 mb-1">{{ __('messages.choose_file_label') }}</label>
        <input
            type="file"
            name="file"
            id="file"
